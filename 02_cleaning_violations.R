...
violations_2022_2025 <- read.csv(paste0(violations_path, 'violations_2022_2025.csv'))
violations_2014_2018 <- read.csv(paste0(violations_path, 'violations_2014_2018.csv'))

list_of_violations_dfs <- list(violations_2014_2018 = violations_2014_2018,
                               violations_2022_2025 = violations_2022_2025)
...
violations_combined <- bind_rows(violations_2014_2018, violations_2022_2025) %>%
...
write.csv(junk_bins, "viol_junk_bins_13mar2026.csv")

####BRINGING BACK GEOCODED DATA         ########################################

geocoding_folder <- "/Users/nataliebrown/Desktop/housing_quality_nyc/outputs/geocoded/"

violations_by_add <- fread(paste0(geocoding_folder, 'viol_junk_bins_13mar2026_SUCCESS_E.csv')) %>%
  select(-c(V1, V10))

new_names_by_add<- c("ViolationID", "borough", "class", "house_number","street", "zipcode",
...
viol_year <- viol_year %>%
  group_by(BIN, viol_year) %>%
  mutate(viol_total = sum(A,B,C,I))

write.csv(viol_year, 'viol_by_year_13mar2026.csv', row.names=F)
